...
class Torrent:
    ...
    def get_by_playlist_id(self, playlist_id: int):
        tracker_data = self.tracker.get_playlist_id(playlist_id=playlist_id, perPage=self.perPage)
    ...
    def get_by_collection_id(self, collection_id: int):
        tracker_data = self.tracker.get_collection_id(collection_id=collection_id, perPage=self.perPage)
        console.log(
    ...
    def get_by_freeleech(self, freeleech: int):
        tracker_data = self.tracker.get_freeleech(freeleech=freeleech, perPage=self.perPage)
    ...
    def get_by_season(self, season: int):
        tracker_data = self.tracker.get_season_number(se_number=season, perPage=self.perPage)
        console.log(f"Seasons torrents.. Filter by the torrent's seasons.. '{season}'")
        if tracker_data:
            self.page_view(tracker_data=tracker_data, tracker=self.tracker)

    def get_by_episode(self, episode: int):
        tracker_data = self.tracker.get_episode_number(ep_number=episode, perPage=self.perPage)
    ...
    def get_refundable(self):
        tracker_data = self.tracker.get_refundable(refundable=True, perPage=self.perPage)
    ...
    def get_personal(self):
        tracker_data = self.tracker.get_personal_release(personalRelease=True, perPage=self.perPage)
